...
const findSewaReport = async (id) => {
  try {
    ...
    const indikasis = pembandingsFix.map((pembanding) => ({
    ...
    const elementPembanding = calculateElementPembanding(tanahs, bangunans, pembandingsFix, persen);
    const karakterFisik = calculateKarakterFisik(tanahs, pembandingsFix, persenMapKarakterFisik);
    const totalPersen = await getTotalPersen(id);
    const totalPenyesuaianElementPembanding = elementPembanding[0]["total_penyesuaian"] || 0;
    const totalPenyesuaiankrakterFisik = karakterFisik[0]["total_penyesuaian"] || 0;
    const totalPenyesuaian = totalPenyesuaianElementPembanding + totalPenyesuaiankrakterFisik;
...
const calulatePersen = (
  ...
  switch (props.label) {
    ...
    case "Luas Bangunan":
      return ((luas_bangunan - luas_bangunan_m2) / luas_bangunan_m2) * props.raw_persen;
    default:
...
const updatePenyesuaianKarakterFisikBySewaId = async (sewaId, data) => {
  try {
    ...
    throw new Error("Error updating penyesuaian karakter fisik: " + error.message);
  }
};
...
const updatePenyesuaianElemenPerbandingBySewaId = async (sewaId, data) => {
  try {
    ...
    //   persen = ((pembandingLuasTanah - object.luas_tanah) / object.luas_tanah) * data.raw_persen;
    //   console.log(
    ...
    await db.query(
      ...
      [data.raw_persen, data.raw_persen, sewaId, data.label, data.pembanding_id]
    );
  } catch (error) {
    throw new Error("Error updating penyesuaian karakter fisik: " + error.message);
  }
