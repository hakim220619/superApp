...
function calculateElementPembanding(
  ...
  const items = [
    {
      label: "Hak Atas Properti yang dialihkan",
...
function calculateLokasi(
  ...
  const getPenyesuaian = (label, pb) => {
    ...
    const indikasi = pb.unit_perbandingan?.indikasi_sewa_m2 || 0;
    const result = (persenVal * indikasi) / 100;
    return { result, raw_persen: rawPersen, persen: persenVal };
  };
  ...
  const items = [
    {
      ...
      objects: objectTanah.length
        ? objectTanah
        : [{ keterangan: "Jarak dari pusat kota", deskripsi: "-" }],
      pembanding: pembandingRows.map((pb) => {
        const { result, raw_persen, persen } = getPenyesuaian("Jarak terhadap pusat kota", pb);
        return {
          pembanding_id: pb.id,
    ...
    },
    {
      ...
      pembanding: pembandingRows.map((pb) => {
        const { result, raw_persen, persen } = getPenyesuaian("Perkerasan Jalan/Lebar Jalan", pb);
        return {
          pembanding_id: pb.id,
    ...
    },
    {
      ...
      objects: tanahRows.map((t) => ({
        keterangan: "Aksesibilitas dan lokasi aset",
        deskripsi: `${t.aksesibilitas_lokasi || "-"}`,
      })),
      pembanding: pembandingRows.map((pb) => {
        const { result, raw_persen, persen } = getPenyesuaian("Aksesibilitas & Lokasi", pb);
        return {
          pembanding_id: pb.id,
          deskripsi: pb.aksesibilitas_lokasi || "-",
          persen: persen || 0,
          raw_persen: raw_persen || 0,
          penyesuaian: rupiah(result),
          _value: result,
    ...
    },
    {
      ...
      objects: objectBangunan.length
        ? objectBangunan
        : [{ keterangan: "Kondisi lingkungan sekitar", deskripsi: "-" }],
      pembanding: pembandingRows.map((pb) => {
        const { result, raw_persen, persen } = getPenyesuaian("Kondisi Lingkungan", pb);
        return {
          pembanding_id: pb.id,
          deskripsi: pb.kondisi_lingkungan || "-",
          persen: persen || 0,
          raw_persen: raw_persen || 0,
          penyesuaian: rupiah(result),
          _value: result,
    ...
    },
    {
      label: "Posisi Aset",
      objects: tanahRows.map((t) => ({
        keterangan: "Posisi atau letak objek",
        deskripsi: `${t.posisi_aset || "-"}`,
      })),
      pembanding: pembandingRows.map((pb) => {
        const { result, raw_persen, persen } = getPenyesuaian("Posisi Aset", pb);
        return {
          pembanding_id: pb.id,
